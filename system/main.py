...
from flcore.servers.serverdpsur import FedDPSUR # newly added
from flcore.servers.serverdpcontrol import FedDPCONTROL # newly added
from flcore.servers.serverpFedMe import pFedMe
from flcore.servers.serverperavg import PerAvg
from flcore.servers.serverprox import FedProx
from flcore.servers.serverprox_ctrl import FedProx_Ctrl # newly added
...
from flcore.servers.serverdyn_ctrl import FedDyn_Ctrl
from flcore.servers.servermoon import MOON
from flcore.servers.servermoonctrl import MOONCTRL
from flcore.servers.serverbabu import FedBABU
from flcore.servers.serverapple import APPLE
from flcore.servers.servergen import FedGen
from flcore.servers.servergenctrl import FedGenCtrl
...
from flcore.servers.serverntdctrl import FedNTDCtrl
...
from utils.result_utils_new import average_data
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif args.algorithm == "FedDPCONTROL":
            server = FedDPCONTROL(args, i)

        elif args.algorithm == "FedDPSUR":
            server = FedDPSUR(args, i)
        ...
        elif args.algorithm == "FedProx_Ctrl":
            server = FedProx_Ctrl(args, i)
        ...
        elif args.algorithm == "FedDyn_Ctrl":
            server = FedDyn_Ctrl(args, i)
        ...
        elif args.algorithm == "MOONCTRL":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = MOONCTRL(args, i)
        ...
        elif args.algorithm == "FedGenCtrl":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedGenCtrl(args, i)
        ...
        elif args.algorithm == "FedNTDCtrl":
            server = FedNTDCtrl(args, i)
    ...
    average_data(dataset=args.dataset, algorithm=args.algorithm, model=args.model_n, goal=args.goal, CRD = args.CRD, crossfade = args.crossfade,
                 kalman_filter = args.kalman_filter, LDP_mechanism = args.LDP_mechanism, clipping = args.clipping,
                 privacy_budget = args.privacy_budget, times=args.times)
    print("All done!")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-model', "--model_n", type=str, default="cnn")
    ...
    parser.add_argument('-dpsur', "--dpsur", type=bool, default=False,
                        help="DPSUR")
    parser.add_argument('-dp', "--privacy", type=bool, default=False,
                        help="differential privacy")
    parser.add_argument('-dps', "--dp_sigma", type=float, default=0.0) # standard devition of the DP noise
    parser.add_argument('-sfn', "--save_folder_name", type=str, default='items')
    parser.add_argument('-ab', "--auto_break", type=bool, default=False)
    parser.add_argument('-dlg', "--dlg_eval", type=bool, default=False)
    parser.add_argument('-dlgg', "--dlg_gap", type=int, default=100)
    parser.add_argument('-bnpc', "--batch_num_per_client", type=int, default=20)
    ...
    ### DP parameters ###
    parser.add_argument('-crd', '--CRD', type=str, default='CRD', help="CRD or Non-CRD") # communication rounds discounting (CRD)
    parser.add_argument('-ldp', '--LDP_mechanism', type=str, default='gaussian', help="gaussian or laplace")
    parser.add_argument('-crf', '--crossfade', type=str, default='False', help="True or False") # crossfade between the loss functions to smoothly train the model
    parser.add_argument('-kf', '--kalman_filter', type=str, default='False', help="True or False")
    parser.add_argument('-pb', '--privacy_budget', type=float, default=10, help="rounds of training")
    parser.add_argument('-delta', '--delta', type=float, default=1e-5, help="DP parameter")
    parser.add_argument('-clipthr', '--clipthr', type=float, default=0.5, help="clipping threshold")
    parser.add_argument('-Dclip', '--clipping', type=str, default='False',
                        help="True or False")  # clipping for the delta loss function value
