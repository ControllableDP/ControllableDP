...
import time
from flcore.clients.clientdpcontrol import clientDPCONTROL
...
class FedDPCONTROL(Server):
    def __init__(self, args, times):
        super().__init__(args, times)

        # select slow clients
        self.set_slow_clients()
        self.set_clients(clientDPCONTROL)
    ...
    def train(self):
        for i in range(self.global_rounds+1):
            ...
            print('-' * 25, 'privacy cost', '-' * 25, self.Privacy_Budget_left[-1][0])
        ...
        if self.num_new_clients > 0:
            ...
            self.set_new_clients(clientDPCONTROL)
            print(f"\n-------------Fine tuning round-------------")
